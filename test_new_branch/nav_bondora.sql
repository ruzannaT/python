...
DROP PROCEDURE IF EXISTS sp_nav_bondora $$
CREATE PROCEDURE sp_nav_bondora (IN toDate DATE)
BEGIN

SELECT 'Bondora' AS platform, hist.loanId, hist.investmentId, inv.myInvestments, hist.outstandingPrincipal, hist.paymentStatus, hist.delayOfPayment,
	CASE
...
FROM BondoraInvestmentHistoryV2 hist LEFT JOIN BondoraInvestmentV2 inv ON inv.loanId=hist.loanId
...
-- CALL sp_nav_bondora('2019-10-29');
